...
const AuthorListInner = ({ authors, onClick }: AuthorListInnerProps) => {
  const fullNames = authors.map(
    (author, idx) => {
      ...
      if (!!onClick && !author?.inactive && authorId !== undefined) {
        return (
          <a
            href={`/authors/${authorId}-${firstName}-${lastName}`} // FIXME: possible href to be sent from Esp.?
  ...
  switch (fullNames.length) {
    case 0: return <></>;
    case 1: return <span className={styles.author} >By {fullNames[0]}</span>;
    case 2: return <span className={styles.author} >By {fullNames[0]} and {fullNames[1]}</span>;
    default: {
      const firstPart = fullNames.slice(0, -1).reduce((acc, item) => [...acc, item, ', '], [] as (string | JSX.Element)[]);
      return <span className={styles.author} >By {firstPart}and {fullNames.slice(-1)}</span>;
    }
  }
};
